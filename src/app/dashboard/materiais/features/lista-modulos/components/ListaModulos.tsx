...
import FuncoesTabela from "@/app/dashboard/materiais/components/FuncoesTabela";
...
export default function ListaModulos({
  ...
  const { itensPaginados, paginaAtual, setPaginaAtual, totalPaginas } =
    usePaginacao(itensPorPagina, modulosEditaveis);
  ...
  const handleCabideChange = (index: number, novoSN: string) => {
    ...
    novos[index] = {
      ...
      Disponibilidade_do_Cabide:
        cabideSelecionado?.Disponibilidade ||
        novos[index].Disponibilidade_do_Cabide,
    };

    setModulosEditaveis(novos);
  };

  const getOmSelectValue = (
    mod: ModuloEditado,
    campo: "omOrigemId" | "omDestinoId"
  ) => {
    if (mod.editando) {
      ...
      {loading || !modulosEditaveis ? (
        ...
        <div className="tabela-wrapper">
              ...
              {itensPaginados.map((mod, idx) => {
                ...
                return (
                  <tr key={realIndex}>
                    ...
                    <td>{mod.SN}</td>
                    <td
                      className={`status ${mod.Disponibilidade.toLowerCase()}`}
                    >
                      {mod.editando ? (
                        <select
                    ...
                    <td>
                      {mod.editando && podeEditar("modulos", "cabideSN") ? (
                        <select
                          ...
                          onChange={(e) =>
                            handleCabideChange(realIndex, e.target.value)
                          }
                          className="select-disponibilidade"
                        >
                    ...
                    <td>
                      {mod.editando ? (
                        <input
                          ...
                          value={mod.Obs}
                          onChange={(e) =>
                            handleObsChange(realIndex, e.target.value)
                          }
          ...
          <div className="info-paginacao">
            Mostrando {itensPaginados.length} de {modulosEditaveis.length}{" "}
            modulos
          </div>
